                ...
                <div className="max-w-4xl mx-auto text-center">
                    <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-medium mb-6 md:mb-10 inline-flex flex-col items-center" style={{ animationDelay: '0.3s', animationFillMode: 'both' }}>
                        <div className="flex items-center">
                            <span className="border border-black px-3 md:px-6 py-2 md:py-4 animate-fade-in" style={{ animationDelay: '0.3s', animationFillMode: 'both' }}>{t('discover')}</span>
                            <span className="bg-[#ff6bff] border border-black px-3 md:px-6 py-2 md:py-4 rounded-[20px] md:rounded-[40px] -ml-px animate-fade-in" style={{ animationDelay: '0.4s', animationFillMode: 'both' }}>{t('events')}</span>
                        </div>
                        <div className="flex items-center -mt-px">
                            <span className="border border-black px-3 md:px-6 py-2 md:py-4 animate-fade-in" style={{ animationDelay: '0.5s', animationFillMode: 'both' }}>{t('near')}</span>
                            <span className="border border-l-0 border-black px-3 md:px-6 py-2 md:py-4 animate-fade-in" style={{ animationDelay: '0.6s', animationFillMode: 'both' }}>{t('you')}</span>
                        </div>
                    </h1>
            ...
            <section id="events-section" className="px-4 md:px-8 pb-16 pt-6 md:pt-16">
                <div>
                    ...
                    <div className="flex flex-wrap items-center gap-0 mb-6 md:mb-8 animate-fade-in" style={{ animationDelay: '0.8s', animationFillMode: 'both' }}>
                        <h2 className="text-base md:text-lg lg:text-xl font-normal w-full sm:w-auto mb-2 sm:mb-0">{t('browsingIn')}</h2>
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    ...
                                    >
                                        ...
                                        {date ? format(date, "MMM do, yyyy") : <span>{t('pickDate')}</span>}
                                    </button>
                                </PopoverTrigger>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 lg:col-start-2 gap-5">
                            {filteredEvents.length === 0 ? (
                                <div className="col-span-full text-center py-12">
                                    {date ? `${t('noEvents')} for ${date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}` : t('noEvents')}
                                </div>
                            ) : (
